%=========================================================================
%
%                  MIXED LAGRANGIAN BEAM
%   29-09-2011
% Fuerzas inerciales incrementales, distribuidas totales, internas totales
%==========================================================================
function [X,gdof,m,kt,kg,fie,fde,fme,SE,EE,AV] = sbeamli(FE,X,E,N,D,DM,U,Ui,Vel,Acc,FD,FI,FM,SE,EE,el,AV,RT)
...
fde = zeros(12,1);   
if strcmp(FE.dyn{5},'grav=yes')==1 % GRAVITY Loads - Assuming that they act in the inertia centroid (no tangent terms because of inertia excentricity)
    sf=zeros(6,6*nxel); fgravel=zeros(6,1);  nips = 1;   sw = int1d (nips);   % Numerical Integration Points
    fgravel(FE.dyn{7},1) = FE.dyn{6} * X.Dinfo{secid,5} ;  % Element weigth
    for i=1:nips
        for j=1:6
            sf(j,j) = ( 1 - sw(1,i) ) / 2;     sf(j,j+6) = ( 1 + sw(1,i) ) / 2;
        end  
