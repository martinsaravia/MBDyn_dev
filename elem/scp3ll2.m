...
function [X,gdof,m,kt,kg,fie,fde,fme,fae,SE,EE,AV] = scp3ll2(FE,X,E,N,D,DM,U,Ui,Vel,Acc,SE,EE,el,AV,RT)
...
Del = D(1:6,1:6,secid);
% disp('ojo, seccion multiplicada por 10 en scp3ll2');
%-----------------------------------------------------------------------------------------------
...
% Global Rotations
idxnn1 = nn1*3-2 : nn1*3;  
idxnn2 = nn2*3-2 : nn2*3;  
idxnn0 = nn0*3-2 : nn0*3;


R1 = RT(idxnn1,1:3,3);          
R2 = RT(idxnn2,1:3,3);     
R0 = RT(idxnn0,1:3,3); %Total Rotation Matrix

DR1 = RT(idxnn1,1:3,1);         
DR2 = RT(idxnn2,1:3,1);    
DR0 = RT(idxnn0,1:3,1); %Incremental Rotation Matrix
...
if strcmp(formu, 'INCREMENTAL')==1;
    ...
    u0n1 = U(udofn1,X.tstep+1);            
    u0n2 = U(udofn2,X.tstep+1);          
    u0n0 = U(udofn0,X.tstep+1);
    ...
    x0n1 = X0n1  +  u0n1;                  
    x0n2 = X0n2  +  u0n2;           
    x0n0 = X0n0  +  u0n0;     % Posición actual
...
Q = [ R0'          I03        I03             I03         -R0'             skew(X0n1r) * T0    ; 
      I03         R0*T0       I03             I03          I03                  -R0*T0         ; 
      I03          I03        R0'             I03         -R0'             skew(X0n2r) * T0    ; 
      I03          I03        I03            R0'*T0        I03                  -R0*T0         ]; 
...
if strcmp(FE.step{1,3},'DYNAMIC')==1
    ...
    mip1(4:6,4:6) = DMel(4:6,4:6) ; % NO ROTATIONAL MASS
...
% x0acc = Acc(udofn0,X.tstep+1) + R0 * (OME * OME + AO) * X0ri + R0 * accrel;
